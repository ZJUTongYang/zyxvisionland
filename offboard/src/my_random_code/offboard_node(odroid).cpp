...
#define ARRIVE_FLAG_THRESHOLD 2000
...
int main(int argc, char **argv)
{
    ...
    while(ros::ok()){
		...
		ROS_INFO("My Position: %f   %f   %f",current_position.pose.position.x, current_position.pose.position.y, current_position.pose.position.z);
        local_pos_pub.publish(pose);
		if(arm_flag ==1)
            break;
		ROS_INFO("He Position: %f   %f   %f",origin_x, origin_y, origin_z);
    ...
    while(ros::ok()){
        ...
        switch(state)
        {
            ...
            case LAND:
                if( current_position.pose.position.z > (origin_z+1.5) )
                {
                    twist.twist.linear.x = 0;
                    twist.twist.linear.y = 0;
        ...
        ROS_INFO("%d  %lf  %lf  %lf",state,twist.twist.linear.x,twist.twist.linear.y,current_position.pose.position.z - origin_z);
